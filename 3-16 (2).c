...
int main(void)
{
    ...
    BFShowGraphVertex(&graph,1);
    
    for(int i = 1 ; i < N ;i++)
    {
        printf("%d\n",graph.adjList[i].parentnum);
    }
...
int VisitVertex(ALGraph * pg, int visitV, int parentV)
{
	if(pg->visitInfo[visitV] == 0)
	{
		...
		pg->adjList[visitV].parentnum = parentV + 1;
...
void BFShowGraphVertex(ALGraph * pg, int startV)
{
	...
	// 시작 정점 탐색
	VisitVertex(pg, visitV, parentV);

	while(LFirst(&(pg->adjList[visitV]), &nextV) == TRUE)
	{
	    
		if(VisitVertex(pg, nextV, visitV) == TRUE)
		{
		    Enqueue(&queue, nextV);
		}
